...
public class Graphics {
	...
	public void drawSubstring(String text, int start, int end, int x, int y,
			int align) {
		switch (align) {
		case LEFT | TOP:
	...
	public void drawChars(char[] paramArrayOfChar, int index, int count, int x,
			int y, int align) {
		switch (align) {
		case LEFT | TOP:
			y += currentFont.getHeight();
	...
	public void drawRGB(ImageArray image, int x, int y, int align) {
		drawRGB(image.piexls, 0, image.width, x, y, image.width, image.height,
				true, align);
	}

	public void drawRGB(int[] colors, int x, int y, int width, int height,
	...
	public void drawRGB(int[] colors, int offset, int stride, int x, int y,
			int width, int height, boolean hasAlpha, int align) {
		byte ud = 0;
		byte lr = 0;
		if ((align & TOP) == TOP) {
			ud++;
