package test.android.calculator;
...
public class Calculator {
	...
	private String getResult(boolean isCut) {
		if(isCut) {
			int length = result.length() - 1;
			if(result.charAt(length) == '0' && result.charAt(length - 1) == '.') {
	...
	public String appendNumber(int num) {
		...
		case WAIT_STATE:
			...
			isNegative = false;
		case INIT_STATE:
			if(num != 0 || isDecimal || result.length() > 2) {
				state = INPUT_STATE;
				resetResult(num);
			}
			break;
		case INPUT_STATE:
			if(maxLength > result.length()) {
				if(isDecimal) {
					...
					if(num != 0 || result.length() > 2) result.insert(result.length() - 1, num);
				}
			}
			break;
		}
	...
	public String equal() {
		...
		case RESULT_STATE:
			...
			if(state == RESULT_STATE) {
				result.append(formatNumber(resultNumber));
			}
			break;
		}
		return getResult();
	...
	public String sqrt() {
		...
		case RESULT_STATE:
			...
			if(num < 0) {
				...
				state = RESULT_STATE;
				result.append(formatNumber(Math.sqrt(num)));
			}
			break;
		}
		return getResult();
